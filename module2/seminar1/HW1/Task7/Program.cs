...
namespace Task7
{
    class Program
    {
     ...
     Ваш выбор: ";
        ...
        {// TODO: реализовать
            int[] sum = new int[Filials.Length];
            for (int i = 0; i < auto.GetLength(0); i++)
            {
                for (int j = 0; j < auto.GetLength(1); j++)
                {
                    sum[j] = sum[j] + auto[i, j];
                }
            }
            int maxIndex = 0;
            int max = 0;
            ...
            NFiliala_MaxAutoYear = maxIndex;
            MaxAutoFilialZaGod = max;
        }
        ...
        {
            int[] sum = new int[Kvartal.Length];
            for (int i = 0; i < auto.GetLength(0); i++)
            {
                for (int j = 0; j < auto.GetLength(1); j++)
                {
                    sum[i] = sum[i] + auto[i, j];
                }
            }
            int maxIndex = 0;
            int max = 0;
